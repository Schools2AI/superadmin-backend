...
export const getRoleIdFromAdminRole = async (role, connection = null) => {
...
export const insertRole = async (connection = null, role) => {
    const db = connection || pool;
    const sql = `INSERT INTO roles (role) VALUES (?)`;
    const [results] = await db.execute(sql, [role]);
