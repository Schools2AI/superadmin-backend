...
const setupDatabase = async () => {
    ...
    CONSTRAINT fk_users_role 
        ...
        const [features] = await pool.query("SELECT COUNT(*) as count FROM features");
        ...
        if (rolePerms[0].count === 0) {
            const [adminRole] = await pool.query("SELECT id FROM roles WHERE role = 'SUPER_ADMIN' LIMIT 1");
